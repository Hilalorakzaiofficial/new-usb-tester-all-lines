...
void drawGraphGrid() {
    ...
    // Vertical Grid Lines
    for (int x = 0; x <= gW; x += 16) tft.drawLine(x, 0, x, gH, COLOR_GRAY);
    
    // Horizontal Grid Lines at Fixed 0.5V steps (0, 0.5, 1.0, 1.5, 2.0)
    // We use a baseline of y=105 for 0.0 and y=5 for 2.0 (100px total height)
    for (int i = 0; i <= 4; i++) {
        int yPos = 5 + (i * 25);
        tft.drawLine(0, yPos, gW, yPos, COLOR_GRAY);
    }
    tft.drawLine(0, 105, gW, 105, COLOR_GRAY); // Baseline grid
    
    tft.drawRect(0, 0, gW, gH, COLOR_BLACK);
    
    tft.setTextSize(1);
    tft.setTextColor(COLOR_BLACK);
    // Labels 0.0 to 2.0 strictly fixed
    tft.setCursor(2, 2);   tft.print("2.0");
    tft.setCursor(2, 27);  tft.print("1.5");
    tft.setCursor(2, 52);  tft.print("1.0");
    tft.setCursor(2, 77);  tft.print("0.5");
    tft.setCursor(2, 101); tft.print("0.0"); // Slightly above baseline 105
}
...
void runGraphSequence() {
    ...
    for (int i = 0; i < NUM_LINES; i++) {
        if (cancelTest) break;
        tft.fillRect(0, 0, 160, 128, COLOR_BG);
        drawGraphGrid();
        ...
        int gW = 160, gH = 112;
        int lx = 0, ly = 105; // Baseline at y=105
        ...
        while (millis() - st < TEST_TIME_GRAPH) {
            ...
            // Scaling 0-2.0V to 100px (105 to 5)
            // y = 105 - (plotV * 50)
            int cy = 105 - (int)(plotV * 50.0);
